...
export function RectangularCalculator() {
    ...
    ? [
        ...
        ...(state.feedYOffset != null
          ? (() => {
              ...
              const RinAtFeed =
                yFromEdge >= 0 && yFromEdge <= result.L
          ...
          ? [
              {
                ...
                value: `${state.feedXOffset != null ? state.feedXOffset.toFixed(3) : '0'}, ${
                  state.feedYOffset != null
                    ? state.feedYOffset.toFixed(3)
                    : result.y0_50ohm != null
                      ? Number(getLengthInUnit(result.y0_50ohm - result.L / 2, lenUnit)).toFixed(3)
                      : '—'
                }`,
                unit: lenUnit,
              },
  ...
  return (
    <section className="calculator-section">
          ...
          {feedExpanded && (
            <>
              <p className="note" style={{ marginTop: '0.5rem', marginBottom: '0.25rem' }}>
                Feed offset only affects the feed-point impedance estimate; W, L, f_r and directivity do not depend on feed
                position.
              </p>
              <label>
      ...
      {result && <ResultsTable title="Output" rows={rows} />}
    </section>
  );
}
